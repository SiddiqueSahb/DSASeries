...
//Approach
/*
1. At each level replacing node value with its depth (i.e root node is replace with 0)
   Node at leve; 1 will we be replace by value 1 and so on.
2.Replacing root node with value 0 and then replacing subtree value by calling recursion and depth value
of subsequent level node will increased by 1.
