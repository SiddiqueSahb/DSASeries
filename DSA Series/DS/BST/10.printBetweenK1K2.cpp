...
// if Node value is greater than min range value and max range value then no need to go to right subtree
if(root->data > k1 && root->data > k2){
	elementsInRangeK1K2(root->left,k1,k2);
}
	...
	// root data is in range of both k1 and k2 so call both left and right
	elementsInRangeK1K2(root->left,k1,k2);
	cout<<root->data<<" ";
	elementsInRangeK1K2(root->right,k1,k2);
