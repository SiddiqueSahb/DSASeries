...
6.DO THIS FOR LST(root->left->left) AND RST(root->right)
*/
...
void insertDuplicateNode(BinaryTreeNode<int> *root) {
   ...
   // if tree is empty then return 
    ...
    // If root is having leftChild then creating a duplicateNode to store root Data .
    // Now create tempNode to store root's leftChild so that rootNode and its leftChild connection
    // doesn't get disconnected.
    //Attach duplicateNode as root's leftChild then to duplicateNode attach root's leftChild store in tempNode
    if(root->left != NULL){
    ...
    // Case : If root is not having leftChild and having rightChild
    // Currently at root's and it is having rightChild, create a duplicateNode with rootData
    //  Attach duplicateNode as root's leftChild
    else{
       BinaryTreeNode<int>* duplicate = new BinaryTreeNode<int>(root->data);
        root->left = duplicate;
    }
    //Inserting Duplicate in LST
    //As we are attaching duplicate node as leftChild so nextNode for which Duplicate is made
    //will be root's left of left
    insertDuplicateNode(root->left->left);
    //inserting duplicate in RST
