...
//Approach
/*
1. inserting at start and deletion at start
...
template <typename T> //each class will have separate template
class Stack {
    Node<T> *head;
    int size; //no of elements present in a stack
    ...
    public :
        ...
        T pop(){
            //before deleting node checking whether stack is empty or not
            if(isEmpty()){
                return 0;
            }
            T ans = head->data;
