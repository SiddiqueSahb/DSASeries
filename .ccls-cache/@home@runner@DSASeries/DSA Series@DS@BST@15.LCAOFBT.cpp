...
5.if both LST AND RST are providing LCA then LCA will be Root
...
int getLCA(BinaryTreeNode <int>* root , int a, int b) {
	...
	int lstAns = getLCA(root->left,a,b);
	int rstAns = getLCA(root->right,a,b);
	if(lstAns != -1 && rstAns == -1){
		return lstAns;
	}else if(lstAns == -1 && rstAns != -1){
		return rstAns;
	}else if(lstAns != -1 && rstAns != -1){
		return root->data;
	}
