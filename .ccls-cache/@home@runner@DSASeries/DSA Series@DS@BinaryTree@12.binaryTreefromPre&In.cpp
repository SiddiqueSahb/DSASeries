...
//APPROACH
/*
1.We are given preOrder and Inorder we have have to construct tree from it
  PreOrder - Root Left Right
  InOrder - Left Root Right
2.Initially we are finding root from PreOrder as root is 1st element present in preOrder  
3.Find rootIndex in Inorder by comparing the root we got from the preOrder
4.creating node for root that we pick from the preorder so that lst and rst can be connected to it 
5.Finding inOrder and preOrder for both LST and RST 
6.calculation for LST for finding from where inorder  and preorder starts and ends
int leftinStart = inStart;  //left inorder start
 ...
 int rightinStart = rootIndex + 1;  // right inOrder start
  ...
  8.Calling recursion to solve the subproblems and return the node which will get connect to root's left and right.
